...
////////////////////////////// 变量解释 //////////////////////////////
// target：如"cpu load cpu-percent=60"中的cpu
// action：如"cpu load cpu-percent=60"中的load
////////////////////////////////////////////////////////////////////
...
var modelActionFlags = make(map[string][]spec.ExpFlag) //key为target+action，value为action后的flag。示例：cpu-percent=60
	...
	if len(args) < 4 {
		...
		expModel := &spec.ExpModel{
			...
			ActionFlags: func() map[string]string {
				flagsx := modelActionFlags[target+action] //如 cpu load

				flagsValues := make(map[string]*string, len(flagsx))

				cmd := flag.NewFlagSet(os.Args[0], flag.ExitOnError) //解析命令行
